...
def ws(wsgi_live_server):
    ...
    service_url = f"{base_url}/ws/jparedes/libreria"
...
def test_wsdl_metadata(ws):
    ...
    assert ws.wsdl.tns == ("tns", "libreria")

    # Inspect the service definition
    sd = ws.sd[0]
    assert sd.service.name == "LibreriaService"
    # We have one port, named LibreriaService, that points to the root of the service ...
    assert len(sd.service.ports) == 1
    svc_port = sd.service.ports[0]
    assert svc_port.name == "LibreriaService"
